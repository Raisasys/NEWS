...
partial class XExtensions
{
    ...
    static readonly ConcurrentDictionary<Assembly, ConcurrentDictionary<Type, IEnumerable<Type>>>
        DescendantsTypesCache = new();
    ...
    public static IEnumerable<Type> WithAllParents(this Type @this)
    {
        ...
        if (@this.BaseType != null)
            foreach (var p in @this.BaseType.WithAllParents())
                yield return p;
    }
        ...
        return TypeImplementsCache.GetOrAdd(key, t =>
        {
            if (!interfaceType.IsInterface)
                throw new ArgumentException(
                    $"The provided value for interfaceType, {interfaceType.FullName} is not an interface type.");
    ...
    /// </summary>
    public static void SetValue(this PropertyInfo @this, object @object, object value) =>
        @this.SetValue(@object, value, null);
    ...
    /// </returns>
    public static bool ExactlyDefines<TAttribute>(this MemberInfo @this, bool inherit = true)
    ...
    public static T Cache<T>(this MethodBase @this, object instance, object[] arguments, Method<T> methodBody)
        where T : class
    {
    ...
    public static IEnumerable<Type> GetSubTypes(this Assembly @this, Type baseType, bool withDescendants = false)
    {
        ...
        return cache.GetOrAdd(baseType, bt =>
        {
            ...
            {
                throw new("Could not load the types of the assembly '{0}'. Type-load exceptions: {1}".FormatWith(
                    @this.FullName,
    ...
    /// Gets the full programming name of this type. Unlike the standard FullName property, it handles Generic types properly.
    /// </summary>
    public static string GetProgrammingName(this Type @this, bool useGlobal, bool useNamespace = true,
        bool useNamespaceForParams = true, bool useGlobalForParams = false, bool useCSharpAlias = false)
    {
        ...
        if (@this.GetGenericArguments().Any())
        {
            return "global::".OnlyWhen(useGlobal && @this.FullName != null) +
                   "{0}{1}<{2}>".FormatWith(
                       ...
                       @this.GetGenericArguments().Select(t => t.GetProgrammingName(useGlobalForParams,
                               useNamespaceForParams, useNamespaceForParams, useGlobalForParams, useCSharpAlias))
                           .ToString(", "));
        }
    ...
    {
        if (@this is PropertyInfo asProp)
        {
            asProp.SetValue(obj, value);
            return;
        }

        if (@this is FieldInfo asField)
        {
    ...
    public static PropertyInfo SafeGetProperty(this Type @this, string propertyName)
    {
        ...
        var result = @this.GetProperty(propertyName,
