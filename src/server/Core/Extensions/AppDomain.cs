...
partial class XExtensions
{
    ...
    public static Type[] FindImplementers(this AppDomain @this, Type interfaceType, bool ignoreDrivedClasses)
    {
                     ...
                     .Where(a => a == interfaceType.Assembly || a.References(interfaceType.Assembly)))
        {
            try
            {
                //Log.For(typeof(XExtensions)).Info($"Looking for {interfaceType.FullName} in " + assembly.FullName);

                foreach (var type in assembly.GetTypes())
                {
                    //Log.For(typeof(XExtensions)).Info($"Checking {type}");
                    if (type == interfaceType) continue;
                    if (type.IsInterface) continue;

                    if (type.Implements(interfaceType))
                        result.Add(type);
                    //else Log.For(typeof(XExtensions)).Info($"{type} does not implement " + interfaceType.FullName);
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine(ex.ToString());
                //Log.For(typeof(XExtensions)).Info($"Could not load assembly {assembly.FullName}");

                //Log.For(typeof(XExtensions)).Info($"Could not load assembly {assembly.FullName} because: {ex.Message}");

                //Log.For(typeof(XExtensions)).Info($"Could not load assembly {assembly.FullName} because: {ex.ToFullMessage()}");
