...
partial class XExtensions
{
            ...
            if (item is DictionaryEntry d)
            {
                ...
                if (d.Key.ToStringOrEmpty() == "ConnectionString")
                    value = value.Remove(" ").Split(';').Trim()
                        .Select(x =>
                            x.ToLower().StartsWithAny("pwd=", "password=") ? (x.RemoveFromAfter("=") + "*****") : x)
                        .ToString("; ");
    ...
    /// </summary>
    public static string ToFullMessage(this Exception @this, string additionalMessage, bool includeStackTrace,
        bool includeData)
    {
        ...
        while (err != null)
        {
            ...
            try
            {
                resultBuilder.AppendLineIf(err.Message);
            }
            catch
            {
            }
    ...
    public static bool IsNetworkRelated(this Exception ex)
    {
        ...
        if (ex.Message.StartsWith("An SSL error has occurred and a secure connection to the server cannot be made."))
            return true;
