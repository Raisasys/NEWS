...
partial class XExtensions
{
    ...
    /// <param name="propertyMappings">An anonymouse object containing property mapping information.
    /// e.g.: new {Property1 = "Property name in CSV", Property2 = "...", set_Property1 = new Func&lt;string, object&gt;(text => Client.Parse(value)) }</param>
    public static Dictionary<T, DataRow> CastAsDictionary<T>(this DataTable @this, object propertyMappings)
        where T : new()
    {
        if (propertyMappings != null)
            foreach (var p in propertyMappings.GetType().GetProperties())
            {
                ...
                throw new ArgumentException(
                    $"Unrecognized value for the property {p.PropertyType} of the specified propertyMappings");
            }
        ...
        if (propertyMappings != null)
            convertors = propertyMappings.GetType().GetProperties()
                .Where(p => p.PropertyType == typeof(Func<string, object>))
    ...
    static Dictionary<string, string> FindPropertyMappings(Type targetType, DataColumnCollection columns,
        object declaredMappings)
    {
        var result = new Dictionary<string, string>();

        if (declaredMappings != null)
        {
            foreach (var property in declaredMappings.GetType()
        ...
        foreach (var property in targetType.GetProperties(System.Reflection.BindingFlags.Instance |
                                                          System.Reflection.BindingFlags.Public))
        {
            ...
            {
                throw new(
                    "The specified data contains multiple potential matches for the property '{0}'. The potentially matched columns found: {1}. You must use explicit mappings in this case."
    ...
    /// </summary>
    static T ParseObject<T>(DataRow dataContainer, Dictionary<string, string> propertyMappings,
        Dictionary<string, Func<string, object>> convertors)
    {
