...
partial class XExtensions
{
    ...
    public static async Task DeleteAsync(this DirectoryInfo @this, bool recursive, bool harshly)
    {
        ...
        if (harshly && !recursive)
            throw new ArgumentException(
                "For deleting a folder harshly, the recursive option should also be specified.");
    ...
    static async Task HarshDelete(DirectoryInfo directory)
    {
        ...
        {
            await directory.GetFiles().Do(async (f) => await f.DeleteAsync(harshly: true).ConfigureAwait(false))
                .ConfigureAwait(false);
            await directory.GetDirectories().Do(async (d) => await HarshDelete(d).ConfigureAwait(false))
                .ConfigureAwait(false);
