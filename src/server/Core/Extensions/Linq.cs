...
partial class XExtensions
{
    ...
    public static IEnumerable<T> Except<T>(this IEnumerable<T> @this, List<T> itemsToExclude) =>
        @this.Except(new HashSet<T>(itemsToExclude));
    ...
    public static IEnumerable<T> Except<T>(this IEnumerable<T> @this, IEnumerable<T> itemsToExclude,
        bool alsoDistinct = false)
    {
    ...
    /// Gets all Non-NULL items of this list.
    /// </summary>
    public static IEnumerable<bool> ExceptNull(this IEnumerable<bool?> @this) =>
        @this.Where(i => i.HasValue).Select(x => x.Value);
    ...
    /// Gets all Non-NULL items of this list.
    /// </summary>
    public static IEnumerable<Guid> ExceptNull(this IEnumerable<Guid?> @this) =>
        @this.Where(i => i.HasValue).Select(x => x.Value);
    ...
    public static int Count<T>(this IEnumerable<T> @this, Func<T, int, bool> criteria) =>
        @this.Count((x, i) => criteria(x, i));
    ...
    /// </summary>        
    /// <param name="items">The item which is searched in the list.</param>
    public static bool LacksAll<T>(this IEnumerable<T> @this, IEnumerable<T> items) =>
        !@this.ContainsAny(items.ToArray());
    ...
    public static bool Intersects<T>(this IEnumerable<T> @this, params T[] items) =>
        @this.Intersects((IEnumerable<T>)items);
    ...
    public static IEnumerable<T> Union<T>(this IEnumerable<T> @this, params T[] otherItems) =>
        @this.Union((IEnumerable<T>)otherItems);
    ...
    public static KeyValuePair<TKey, TValue> Add<TKey, TValue>(this IList<KeyValuePair<TKey, TValue>> @this, TKey key,
        TValue value)
    {
    ...
    [Obsolete(
        "Please use AllIndices<T> instead. This method will return [-1] for non-existing items. This is causing unexpected behaviors and will be removed in the next major version.")]
    ...
    public static bool ContainsAny(this IEnumerable<Guid> @this, params Guid?[] ids) =>
        @this.ContainsAny(ids.ExceptNull().ToArray());
    ...
    public static IEnumerable<TSource> OrderBy<TSource>(this IEnumerable<TSource> @this, string propertyName)
    {
        ...
        if (property == null)
            throw new ArgumentException(
                "{0} is not a readable property of {1} type.".FormatWith(propertyName, typeof(TSource).FullName));

        return @this.OrderBy(
            new Func<TSource, object>((new PropertyComparer<TSource>(property)).ExtractValue<TSource, object>));
    }
    ...
    public static IEnumerable<TSource> OrderByDescending<TSource>(this IEnumerable<TSource> @this, string propertyName)
    {
        ...
        return @this.OrderByDescending(
            new Func<TSource, object>((new PropertyComparer(property)).ExtractValue<TSource, object>));
    }
    ...
    public static Task ForEachAsync<T>(this IEnumerable<T> source, int degreeOfParallelism, Func<T, Task> func)
        => source.ForEachAsync(degreeOfParallelism, (x, _) => func(x));
    ...
    public static Task ForEachAsync<T>(this IEnumerable<T> source, int degreeOfParallelism, Func<T, int, Task> func)
    {
        ...
        return Task.WhenAll(
            partitions.Select((partition, index) => Task.Run(async () =>
            {
                using (partition)
