...
partial class XExtensions
{
    ...
    /// </summary>
    /*public static async Task<byte[]> DownloadData(this Uri @this, string cookieValue = null,
        double timeOutSeconds = MINUTE)
    {
    ...
    public static Task<HttpResponseMessage> Post(this Uri url, object data, Action<HttpClient> customiseClient = null)
    {
        ...
        return client.PostAsync(url.ToString(),
            new FormUrlEncodedContent(new Dictionary<string, string>().AddFromProperties(data)));
    }
    ...
    /// All items in the postData object will be sent as individual FORM parameters to the destination.
    /// </summary>
    public static Task<HttpResponseMessage> Post(this Uri url, Dictionary<string, string> postData,
        Action<HttpClient> customiseClient = null)
    {
        using var client = new HttpClient();
        customiseClient?.Invoke(client);
