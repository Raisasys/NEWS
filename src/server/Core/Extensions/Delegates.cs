...
partial class XExtensions
{
    ...
    public static void Invoke(this Action action, int retries, TimeSpan waitBeforeRetries,
        Action<Exception> onEveryError = null)
    {
        var asFunction = new Func<object>(() =>
        {
            action?.Invoke();
            return null;
        });
    ...
    /// <param name="waitBeforeRetries">The time to wait before every two retries.</param>
    /// <param name="onEveryError">The action to run every time the method invokation fails. You can use this to log the error.</param>
    public static T Invoke<T>(this Func<T> function, int retries, TimeSpan waitBeforeRetries,
        Action<Exception> onEveryError = null)
    {
        if (retries < 2)
