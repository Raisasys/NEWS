...
partial class XExtensions
{
    ...
    public static async Task<byte[]> DownloadData(this HttpClient client, string address, bool handleGzip)
    {
        ...
        if (result != null && result.Length > 3 && result[0] == GZipStarter[0] && result[1] == GZipStarter[1] &&
            result[2] == GZipStarter[2])
        {
            // GZIP:
            using var stream = new System.IO.Compression.GZipStream(new MemoryStream(result),
