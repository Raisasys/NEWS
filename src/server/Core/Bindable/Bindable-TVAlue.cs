...
public partial class Bindable<TValue> : Bindable
{
    ...
    public static bool operator ==(Bindable<TValue> item, object other)
    {
        if (item is null)
            throw new InvalidOperationException(
                "A bindable object should never be null. It should be instantiated upon declaration.");
    ...
    public IBinding<TValue> AddBinding<TProperty>(object target, string propertyName,
        Func<TValue, TProperty> expression)
    {
        var property = FindProperty(target, propertyName);
    ...
    static PropertyInfo FindProperty(object target, string propertyName)
    {
        ...
        if (property is null)
            throw new Exception(
                $"{target.GetType().FullName} does not have a public instance property named {propertyName}.");
